	...
	if (type == LOAD || type == RFO || type == PREFETCH) {
		...
		if (!present) {
			...
			if (   (    ReD_PCRT[cpu][PCentry].reuse_counter * 64 > ReD_PCRT[cpu][PCentry].noreuse_counter
					 && (ReD_PCRT[cpu][PCentry].reuse_counter * 7) < (ReD_PCRT[cpu][PCentry].noreuse_counter)*2)
				|| (misses[cpu] % 8 == 0)) 
			{
				remember(cpu, PC, block);
			}
			// bypass when address not in ART and reuse in PCRT is low or intermediate
			if ((ReD_PCRT[cpu][PCentry].reuse_counter * 7) < (ReD_PCRT[cpu][PCentry].noreuse_counter *2)) {
				/* BYPASS */
				red_result = LLC_WAYS;    
                goto hawkeye_label;                     
			}
