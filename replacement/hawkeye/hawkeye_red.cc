...
uint8_t lookup(uint32_t cpu, uint64_t PCnow, uint64_t block)
{
	...
	for (i=0; i<RED_WAYS; i++) {
		if ((ReD_ART[cpu][ART_set].adds[i].tag == tag) && (ReD_ART[cpu][ART_set].adds[i].valid[subsector] == 1)) {
			if (ART_set % RED_PC_FRACTION == 0 && type_dyn == TYPE_RED) {
	...
	if (i != RED_WAYS) {
		...
		if (ART_set % RED_PC_FRACTION == 0) {
			...
			for (int s=0; s<RED_SECTOR_SIZE; s++) {
				if (ReD_ART[cpu][ART_set].adds[where].valid[s] && type_dyn == TYPE_RED) {
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type){
    ...
    if (type == LOAD || type == RFO || type == PREFETCH) {
        ...
        if (!present) {
            ...
            if (   (    demand_predictor->get_reuse(PCentry) * 64 > demand_predictor->get_noreuse(PCentry)
                        && demand_predictor->get_reuse(PCentry) * 3 < demand_predictor->get_noreuse(PCentry) *1 )
                    || (misses[cpu] % 4 == 0)) 
                {
                    remember(cpu, PC, block);
                }
                // bypass when address not in ART and reuse in PCRT is low or intermediate
                if (demand_predictor->get_reuse(PCentry) * 3 < demand_predictor->get_noreuse(PCentry) *1 ) {
                    /* BYPASS */
...
void UpdateReplacementState (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if(SAMPLED_SET(set))
    {
        ...
        if((addr_history[sampler_set].find(sampler_tag) != addr_history[sampler_set].end()) && (type != PREFETCH))
        {
            ...
            if( !wrap && perset_optgen[set].should_cache(curr_quanta, last_quanta) && (type_dyn == TYPE_HAWK))
            {
        ...
        else //This line is a prefetch
        {
            ...
            {
                if(perset_optgen[set].should_cache(curr_quanta, last_quanta) && (type_dyn == TYPE_HAWK))
                {
                    if(addr_history[sampler_set][sampler_tag].prefetched)
                        prefetch_predictor->increment(addr_history[sampler_set][sampler_tag].PC);
                    else
    ...
    if (type_dyn == TYPE_HAWK){

        if (instr_window == MAX_WINDOW){
            if ((miss_in_window > prev_miss_in_window) && (prev_miss_in_window)){
                cout << "Shifting to ReD" << endl;
                type_dyn = TYPE_RED ;
            }
            prev_miss_in_window = miss_in_window ;
            miss_in_window = 0;
            instr_window = 0;
        }
