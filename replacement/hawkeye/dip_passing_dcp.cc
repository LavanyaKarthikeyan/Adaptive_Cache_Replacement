...
// #define SAMPLED_RED(set) ((set%32) == 26 )
// #define SAMPLED_HAWKEYE(set) ((set%32) == 0 )

// #define SAMPLED_RED(set) (bits(set, 0 , 5) == bits(set, ((unsigned long long)log2(LLC_SETS) - 5), 5) )
// #define SAMPLED_HAWKEYE(set) (bits(set, 0 , 5) == (~(bits(set, ((unsigned long long)log2(LLC_SETS) - 5), 5) )))

#define SAMPLED_HAWKEYE(set) ((set&31) == ((set >> 5)&31))
#define SAMPLED_RED(set) ((set&31) == ((((set >> 5)^31))&31))
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type){
    ...
    if (SAMPLED_RED(set) ||(!select_type && !SAMPLED_HAWKEYE(set))){
        while (1) {
    ...
    //If we cannot find a cache-averse line, we evict the oldest cache-friendly line
     if (SAMPLED_HAWKEYE(set) ||(select_type && !SAMPLED_RED(set))){
        for (uint32_t i=0; i<LLC_WAYS; i++)
        {
    ...
    if (SAMPLED_HAWKEYE(set)){
        return hawk_result ;
    }
    else if (SAMPLED_RED(set)){
...
void PrintStats()
{
    ...
    std::cout << "Total Shifts: " << count << std::endl;
